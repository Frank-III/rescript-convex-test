...
let make = (~roomId: roomsId, ~userId: usersId) => {
  ...
  let messagesRaw = Convex_messages.Query_getByRoom.use(~roomId, ~limit=Some(50.0))
        ...
        if messageList->Array.length === 0 {
          ...
          <div className="space-y-4">
            ...
            ->Array.map(msg => {
              ...
              <div
                key={msg.id} className={`flex ${isOwnMessage ? "justify-end" : "justify-start"}`}
              >
                <div
                ...
                >
                  ...
                  | Some(user) =>
                    if !isOwnMessage {
                      <div className="text-xs font-semibold mb-1"> {React.string(user.name)} </div>
                    } else {
                      React.null
                    }
                  }}
                  ...
                  >
                    {React.string(Date.fromTime(msg.createdAt)->Date.toLocaleTimeString)}
                  </div>
                </div>
              </div>
            })
